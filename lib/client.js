...
function matchRules(rules, item) {
    ...
    return rules.filter((r) => {
        ...
        if (!text.includes(r.keyword.toLowerCase()))
            return false;
        if (r.source && !String(item.source || '').toLowerCase().includes(r.source.toLowerCase()))
            return false;
...
function HomeView(props) {
    ...
    const portalNews = (0, react_1.useMemo)(() => important.filter(isPortalIt).slice(0, 8), [important]);
