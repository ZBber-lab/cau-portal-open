...
export function apply(ctx) {
    ...
    webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                const gh = await fetch(`https://api.github.com/repos/zhouxuanting52-lab/cau-portal/contents/${rel}?ref=main`, {
                    headers: {
    ...
    const portalEngine = () => import(new URL('../tools/portal/engine.mjs', import.meta.url).href);
    webServer.register({
        kind: 'exact',
        path: '/api/cau/portal/status',
        handler: async (_req, res) => {
            try {
                const eng = await portalEngine();
    ...
    webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                if (out.ok)
                    json(res, 200, { ok: true, message: '登录成功', user: username });
                else
    ...
    const portalSync = () => import(new URL('../tools/portal/sync.mjs', import.meta.url).href);
    webServer.register({
        kind: 'exact',
    ...
    portalSync()
        .then((m) => m.startPortalScheduler({
        onResult: (r) => {
            const note = r.ok ? `portal 同步完成（新 ${r.new_items ?? 0} 条）` : `portal 同步失败：${r.error ?? ''}`;
            ctx?.logger?.info(`[cau-portal] ${note}`);
        },
    }))
        .catch((e) => ctx?.logger?.warn(`[cau-portal] portal 调度启动失败：${String(e?.message ?? e)}`));
    ctx?.logger?.info('[cau-portal] server routes registered: /api/cau/health, /api/cau/enrich, /api/cau/data, /api/cau/portal/{status,account,clear,sync}');
}
