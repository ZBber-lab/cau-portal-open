...
function storeDirs() {
    ...
    try {
        for (const name of readdirSync(root)) {
            ...
            if (!out.includes(dir) && existsSync(dir))
                out.push(dir);
        }
    }
    catch {
...
function readStoredToken() {
    for (const dir of storeDirs()) {
        try {
            ...
            if (token)
                return { token, updatedAt: String(j?.updatedAt || ''), dir };
        }
        catch {
            /* 换下一个候选 */
        }
    ...
    webServer.register({
        ...
        handler: async (req, res) => {
            ...
            if (method === 'DELETE') {
                ...
                try {
                    if (cur)
                        unlinkSync(joinPath(cur.dir, STORE_FILE));
